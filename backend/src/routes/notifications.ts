...
notificationsRouter.get('/', requireAuth, async (req, res) => {
  ...
  if (error) return res.status(400).json({ error: 'notifications_failed', details: error.message })
...
notificationsRouter.patch('/:notificationId/read', requireAuth, async (req, res) => {
  ...
  if (error) return res.status(400).json({ error: 'notification_mark_read_failed', details: error.message })
