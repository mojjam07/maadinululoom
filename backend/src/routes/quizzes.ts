...
quizzesRouter.post('/sets', requireAuth, async (req, res) => {
  ...
  if (error) return res.status(400).json({ error: 'quiz_set_create_failed', details: error.message })
...
quizzesRouter.get('/sets/:quizSetId', async (req, res) => {
  ...
  if (error) return res.status(400).json({ error: 'quiz_set_fetch_failed', details: error.message })
...
quizzesRouter.post('/attempts/:quizSetId/start', requireAuth, async (req, res) => {
  ...
  if (error) return res.status(400).json({ error: 'attempt_start_failed', details: error.message })
...
quizzesRouter.post('/attempts/:attemptId/submit', requireAuth, async (req, res) => {
  ...
  const { is_submitted } = req.body as { is_submitted?: boolean }
  ...
  if (error) return res.status(400).json({ error: 'attempt_submit_failed', details: error.message })
  if (!data) return res.status(404).json({ error: 'attempt_not_found' })
