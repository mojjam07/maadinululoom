...
enrollmentsRouter.post('/', requireAuth, async (req, res) => {
  ...
  const { student_id, subject_id, status } = req.body as {
    student_id?: string
    subject_id?: string
    status?: string
  }
  ...
  if (finalStudentId !== userId) return res.status(403).json({ error: 'forbidden' })
  ...
  if (error) return res.status(400).json({ error: 'enrollment_failed', details: error.message })
