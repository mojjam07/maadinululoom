...
function requireAdminOrTeacher(role: string | null): boolean {
...
classesRouter.get('/', requireAuth, async (req, res) => {
  ...
  if (enrErr) return res.status(400).json({ error: 'classes_failed', details: enrErr.message })
  ...
  if (clsErr) return res.status(400).json({ error: 'classes_failed', details: clsErr.message })
...
classesRouter.post('/', requireAuth, async (req, res) => {
  const teacherOrAdminId = (req as any).auth.userId as string
  const role = await getRole(teacherOrAdminId)
  if (!requireAdminOrTeacher(role)) return res.status(403).json({ error: 'forbidden' })
