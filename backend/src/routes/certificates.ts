...
certificatesRouter.post('/issue', requireAuth, async (req, res) => {
  ...
  const _actorId = (req as any).auth.userId as string
  ...
  if (error) return res.status(400).json({ error: 'certificate_issue_failed', details: error.message })
...
certificatesRouter.get('/me', requireAuth, async (req, res) => {
  ...
  if (error) return res.status(400).json({ error: 'certificates_fetch_failed', details: error.message })
...
certificatesRouter.get('/verify/:certId', async (req, res) => {
  ...
  if (error) return res.status(400).json({ error: 'certificate_verify_failed', details: error.message })
