...
assignmentsRouter.get('/:studentId', requireAuth, async (req, res) => {
  ...
  if (enrErr) return res.status(400).json({ error: 'assignments_failed', details: enrErr.message })

  const subjectIds = (enrolled || []).map((e) => e.subject_id)
  if (subjectIds.length === 0) return res.json({ assignments: [] })
  ...
  if (lesErr) return res.status(400).json({ error: 'assignments_failed', details: lesErr.message })

  const lessonIds = (lessons || []).map((l) => l.id)
  if (lessonIds.length === 0) return res.json({ assignments: [] })
  ...
  if (asErr) return res.status(400).json({ error: 'assignments_failed', details: asErr.message })
