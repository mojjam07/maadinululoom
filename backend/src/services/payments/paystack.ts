...
export async function handlePaystackWebhook(req: express.Request, _res: express.Response) {
  // Paystack typical body: { event, data: { ... } }
  const body = req.body as any
  const eventType: string | null = getText(body?.event)
  ...
  const studentId: string | null = getText(data?.metadata?.student_id) || getText(data?.metadata?.customer_id)

  if (!studentId) throw new Error('paystack_webhook_missing_student_id')
  if (!reference) throw new Error('paystack_webhook_missing_reference')
