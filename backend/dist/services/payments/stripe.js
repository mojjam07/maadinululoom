...
async function handleStripeWebhook(req, _res) {
    ...
    const paymentRef = getText(obj?.id) ||
        getText(obj?.payment_intent) ||
        getText(obj?.invoice) ||
        getText(obj?.payment_ref);
    const metadata = obj?.metadata || {};
    const studentId = getText(metadata?.student_id);
    if (!studentId)
        throw new Error('stripe_webhook_missing_student_id');
    if (!paymentRef)
        throw new Error('stripe_webhook_missing_reference');
